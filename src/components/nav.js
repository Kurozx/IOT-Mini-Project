      ...
      <div className="container-fluid">
        ...
        <div className="collapse navbar-collapse" id="navbarSupportedContent">
          ...
          <form className="d-flex align-items-center">
            ...
            <button type="button" className={`btn ${styles.btnDanger}`} onClick={() => updateLEDStatus('OFF_RGB', setLEDStatus)}>RGB OFF</button>
            <button type="button" className={`btn ${styles.btnCustom} me-2`} onClick={() => updateLEDStatus('BUZZER_ON', setLEDStatus)}>On Buzzer</button>
            <button type="button" className={`btn ${styles.btnDanger}`} onClick={() => updateLEDStatus('OFF_BUZZER', setLEDStatus)}>OFF Buzzer</button>
          </form>
        </div>
      </div>
    </nav>
