...
export async function POST(req) {
  ...
  try {
    ...
    await client.query(`INSERT INTO "NCN046" ("command", "date") VALUES ($1, NOW())`, ['RGB_ON']);

    console.log("RGB control command stored in the database");

    // Fetch the latest command from the database
    const result = await client.query(`SELECT "command", "date" FROM "NCN046" ORDER BY "date" DESC LIMIT 1`);
